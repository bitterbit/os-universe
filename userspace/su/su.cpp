...
void pwtoid(const char* tok, uid_t* uid, gid_t* gid) {
    ...
    if (pw) {
        ...
        if (errno != 0 || end == tok) {
            printf("invalid uid/gid '%s' errno: %d\n", tok, errno);
            exit(1);
        }
        if (uid) *uid = tmpid;
        if (gid) *gid = tmpid;
...
int main(int argc, char** argv) {
    ...
    if (current_uid != ID_ROOT && current_uid != ID_SHELL) {
        printf("not allowed if not root or shell\n");
        exit(1);
    } 
    ...
    if (*argv) {
        ...
        if (gids_count) {
            printf("Warning, will set group to first group only... TODO\n");
            gid = gids[0];
        }
        ++argv;
    }
    if (setgid(gid)) {
        printf("setgid failed with errno: %d\n", errno);
        exit(1);
    }
    ...
    if (i == 0) exec_args[i++] = const_cast<char*>("/bin/sh");
    exec_args[i] = NULL;
    execvp(exec_args[0], exec_args);

    printf("failed to exec %s with errno: %d\n", exec_args[0], errno);
    exit(1);
}
